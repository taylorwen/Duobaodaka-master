...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
        ...
        {
            arrOfType = @[@"全部分类",@"手机数码",@"电脑办公",@"家用电器",@"化妆个护",@"钟表首饰",@"其他奖品"];
            arrOfTypeImage = @[@"sort0",@"sort100",@"sort106",@"sort104",@"sort2",@"sort222",@"sort312"];
        }
        else
        {
            arrOfType = @[@"家用电器",@"化妆个护",@"钟表首饰",@"其他奖品"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row == indexType)
    {
        ...
        UIImageView* imgOK = [[UIImageView alloc] initWithFrame:CGRectMake(mainWidth - 40, 14, 20, 20)];
