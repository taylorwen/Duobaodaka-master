...
{
    __weak id<MineBuyingViewDelegate> delegate;
    UIImageView     *imgPro;
    UILabel         *lblTitle;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        lblNum = [[UILabel alloc] initWithFrame:CGRectMake(170, 105, 100, 15)];
        ...
        lblRC.font = [UIFont systemFontOfSize:14];
        [self addSubview:lblRC];
        
        if ([[UserInstance ShardInstnce].versionStatus isEqualToString:@"1"])
        {
            UIButton* btnCart = [[UIButton alloc] initWithFrame:CGRectMake(mainWidth-50, 95, 40, 25)];
            btnCart.layer.borderWidth = 0.5;
            btnCart.layer.borderColor = [UIColor  redColor].CGColor;
            btnCart.layer.cornerRadius = 4;
            btnCart.backgroundColor = [UIColor redColor];
            btnCart.titleLabel.font = [UIFont systemFontOfSize:14];
            [btnCart setTitle:@"追加" forState:UIControlStateNormal];
...
- (void)setBuying:(MineMyBuyItem*)item
{
    ...
    lblRC.frame = CGRectMake(160 + s.width + 10, 105, 100, 15);
}
...
- (void)addCartAction
{
    if(delegate)
    {
        [delegate addGotoCartAction:_myItem];
    }
}
