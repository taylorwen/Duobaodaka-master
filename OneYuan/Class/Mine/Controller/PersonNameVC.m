...
@interface PersonNameVC ()<UITextFieldDelegate>
{
    __block UITextField* txtPhone;
}
@end

@implementation PersonNameVC
...
- (void)viewDidLoad {
    ...
    [btnDone addTarget:self action:@selector(btnChangeUsername) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:btnDone];
    
}

- (void)btnChangeUsername
{
    ...
    NSDictionary* dict = @{@"uid":userId,@"username":txtPhone.text,@"auth_key":[UserInstance ShardInstnce].auth_key,@"timestamp":timestamp,@"token":token};
    [PersonModel doEditUsername:dict success:^(AFHTTPRequestOperation *operation, NSObject *result)
     {
         NSDictionary* dataDict = [(NSDictionary*)result objectForKey:@"data"];
         NSError* error = nil;
         UsernameModel *user = [[UsernameModel alloc]initWithDictionary:dataDict error:&error];
