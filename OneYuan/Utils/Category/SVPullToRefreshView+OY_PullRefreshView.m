...
- (void)setOYStyle
{
    ...
    [self setTitle:@"正在加载" forState:SVPullToRefreshStateLoading];
    [self setSubtitle:@"" forState:SVPullToRefreshStateAll];
    self.tzsCustomView = [[OYSVCustomView alloc] initWithImage:[UIImage imageNamed:@"pulltorefresh_top"]];//旋转处的静态图片
    ...
    {
        UIView *frameView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, mainWidth, 50)];
        frameView.backgroundColor = [UIColor clearColor];
        [self addSubview:frameView];
        
        self.oyCustomView.frame = CGRectMake(0, 0, mainWidth, 50);
        
        [frameView addSubview:self.oyCustomView];
        
        UIImageView *stripView = [[UIImageView alloc] init];
        stripView.tag = kStripViewTag;
        stripView.image = [UIImage imageNamed:@""];//旋转的图片
...
- (void)startAnimatingCustomView {
    if (![self.oyCustomView.layer.animationKeys containsObject:@"rotateAnimation"]) {
        ...
        rotateAnimation.byValue = @(M_PI*3);//调节旋转速度
